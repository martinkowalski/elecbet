"""Unit tests for polls.py"""

import pytest
from src.polls import Poll, pool, from_csv

def test_poll_creation():
    """Test typical poll creation with automatic calculation of Polls.OTHERS_KEY share."""
    pl = Poll('2025-05-15', 800, {'party_a': 0.5, 'party_b': 0.4})
    assert pl.results[Poll.OTHERS_KEY] == pytest.approx(0.1, abs=Poll.TOL)

def test_poll_creation_with_shares_gt_1():
    """Test that creating a poll with total shares > 1.0 raises a ValueError."""
    with pytest.raises(ValueError):
        Poll('2025-05-15', 800, {'a': 0.5, 'b': 0.6})

def test_poll_creation_with_shares_neq_1():
    """Test that including Poll.OTHERS_KEY with incorrect total share raises a ValueError."""
    with pytest.raises(ValueError):
        Poll('2025-05-15', 800, {'a': 0.5, 'b': 0.4, Poll.OTHERS_KEY: 0.9999})
